...
- **Phase 3 (Data Layer): COMPLETE** — awaiting "Phase Approved". Branch `feat/phase-03-data-layer`. Layering locked: screens → hooks → RTK Query (queryFn) → repositories → services. `AppError` taxonomy + `toAppError` funnel; `createHttpClient` + `getWithRetry` (backoff, GETs only); hand-rolled persistence (listener middleware + AppStorage, whitelist: cache slice) rehydrated before first render; `useOnline` + global OfflineBanner; MockDestinationsRepository with failure injection; `/dev-data` harness proves loading/error/empty/data + airplane-mode cold start. Guard RTK errors with `isAppError`. Live-API cache-serving branch still to be proven with real APIs (Phase 5).
- **Next: Phase 4 — Auth & Profile** (Google Sign-In via EAS development build — first dev-build milestone; SecureStore sessions; guest mode. `roava://` scheme becomes testable.)
