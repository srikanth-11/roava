...
- **Current: Phase 10 — Currency** — branch `feat/phase-10-currency`, plan at `docs/plans/phase-10-currency.md`; build not started. Flagship offline-first showcase: converter (whole-table er-api caching per base, keyless), favorite pairs persisted, staleness banner. `currency.ts` + the weather TTL cache are the dress rehearsals.
- Backlog: commitlint graduation; **phone needs a fresh arm64 build** (the sideload APK predates MapLibre/mmkv@3 — rebuild with `-PreactNativeArchitectures=arm64-v8a` when next needed). (Done 2026-07-05: LAN fix — JOURNEY 9.3; MMKV swap — JOURNEY 12.1.)
