...
- **Current: Phase 8 — Weather** — branch `feat/phase-08-weather`, plan at `docs/plans/phase-08-weather.md`; build not started. react-native-svg already compiled into the dev build (no rebuild needed for the sunrise arc). Free-tier reality check in plan Task 1: `/data/2.5/forecast` (3-hourly, 5 days) + `/air_pollution` are free; One Call (true daily + UV) is not — daily aggregates client-side, UV via keyless Open-Meteo or honest degradation.
