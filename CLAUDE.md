...
- **Current: Phase 9 — Maps & Nearby** — branch `feat/phase-09-maps`, plan at `docs/plans/phase-09-maps.md`; build not started. **Stack: MapLibre + OpenFreeMap tiles — zero keys, zero cards, no user action** (Google Maps rejected: needs a billing account, against the free-keys rule). **Needs a native rebuild** (@maplibre/maplibre-react-native config plugin) — the mmkv@3 swap (JOURNEY 7.2 backlog) rides the same rebuild. OSM attribution required on-map. Map degrades to the POI list if init fails.
